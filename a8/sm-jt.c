...
int exec() {
    int cont = 1;
    int addr, val;
    static const void* jt[] = {&&l0, &&l1, &&l2, &&l3, &&l4, &&def, &&l6, &&l7, &&l8, &&l9, &&la, &&lb, &&lc, &&ld, &&le, &&lf};
    static const void* jt1[]= {&&ll0, &&ll1, &&ll2, &&ll3, &&ll4, &&ll5, &&ll6, &&ll7, &&ldef, &&ldef, &&ldef, &&ldef, &&ldef, &&ldef, &&ldef, &&llf};

    if(insOpCode < 0 || insOpCode > 15)
        goto def;

    goto *jt[insOpCode];

    l0: reg [insOp0] = insOpExt;
        goto CONT;
    l1: // ld o(rs), rd .......... 1osd
    ...
    if (insOp0 == 0)
        cont = 0;
    goto CONT;
    def: printf ("Illegal  instruction: pc=0x%x opCode=0x%x\n", pc, insOpCode);
         goto CONT;
    CONT: return cont;
}
